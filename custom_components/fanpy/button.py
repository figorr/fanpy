...
async def async_setup_entry(
    ...
    entities = [
        FanpyFanResyncButton(hass, entry, prefix, name),
    ]
    if entry.data.get(CONF_HAS_LIGHT, False):
        entities.append(FanpyLuzResyncButton(hass, entry, prefix, name))

    async_add_entities(entities)


class FanpyFanResyncButton(ButtonEntity):

    def __init__(self, hass: HomeAssistant, entry: ConfigEntry, prefix: str, name: str) -> None:
        self._entry = entry
        self._prefix = prefix
        self._attr_name = f"Fanpy {name} Resync Ventilador"
...
class FanpyLuzResyncButton(ButtonEntity):

    def __init__(self, hass: HomeAssistant, entry: ConfigEntry, prefix: str, name: str) -> None:
        self._entry = entry
        self._prefix = prefix
        self._attr_name = f"Fanpy {name} Resync Luz"
